function [y2,ntrimmed,spikes, yy, spikelen] = splinetrim(y,varargin)
% function [y,ntrimmed,spikes, yfit, spikelen] = splinetrim(y,[iqrmult],[knotrate],[X],['p'])
%
...
%
% spikelen: index of each observation, whose value is the spike/exclude run
% length. This is useful for identifying long runs of missing data that should be
% excluded
%
%  3/29/05, Tor Wager
%  6/5/09, Tor: update to add spikelen, to exclude long runs rather than
%  interpolating, and pad end values to improve spline fit
...
create_figure('pupil data');
plot(y)
disp('Click so that horizontal line cuts off all values you consider blinks')
[X, blink_thresh] = ginput(1);
manual_spikes = y < blink_thresh;
spikes = spikes + manual_spikes;
...
spikes2 = moving_average('gaussian', spikes, 4);
...
for i = 1:nbp
    ...
    data = ytmp(st:en);
    bpy(i) = nanmedian(data);

end

% need to pad to avoid crazy spline fits
padbp = (bp(end)+10:10:bp(end)+100)';

% xx = 1:length(y);
%yy = spline(bp, bpy, 1:length(y));

yy = spline([bp; padbp], [bpy; repmat(nanmedian(data), length(padbp), 1)], 1:length(y));
