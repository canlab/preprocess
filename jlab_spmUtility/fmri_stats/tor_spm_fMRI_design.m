function [xX,Sess] = spm_fMRI_design(nscan,RT,TOR)
% TOR.c is the cell array of onsets across all columns of design matrix!
...
global torindex
disp(['fMRI_T (time bins per TR) is ' num2str(fMRI_T)])
disp(['fMRI_T0 (reference slice) is ' num2str(fMRI_T0)])

if ~(fMRI_T == TOR.fmriT) | ~(fMRI_T0 == TOR.refslice)
	error('FMRI_T or T0 TIMES DO NOT MATCH INPUT: try declaring them as global vars in workspace...?')
end
...
% tor added as check.
disp(['RT is ' num2str(RT)])
if ~(RT == TOR.RT)
	error('RT DOES NOT MATCH INPUT IN TOR.RT: check inputs?')
end
...
for s = 1:nsess
	...
	if (s == 1) | ~rep
		...
		[SF,Cname,Pv,Pname,DSstr] = ...
			tor_spm_get_ons(k,fMRI_T,dt,STOC,Fstr,[],[],s,TOR);
		ntrial = size(SF,2);
	...
	%---------------------------------------------------------------
		...
		%-------------------------------------------------------
      [BF BFstr] = tor_spm_get_bf(Cname,fMRI_T,dt,Fstr,s,[],TOR);
	end
	...
	%---------------------------------------------------------------
		...
		if ~ntrial
			...
			for   i = 1:ntrial
				...
				% tor modified to deal with empty onset conditions.
				if isempty(on)
					pst = [];
				else
...
% tor: append user-specified regressors as-is.
   %---------------------------------------------------------------
if isfield(TOR,'usregs')
   Xx = [Xx TOR.usregs];
   %whos Xname
   %regr_us_na
   Xname = [Xname TOR.usna];
end
