function [sf,Cname,Pv,Pname,DSstr] = spm_get_ons(k,T,dt,STOC,Fstr,v,Cname,s,TOR)
...
%
%_______________________________________________________________________
% @(#)spm_get_ons.m	2.28 Karl Friston 00/01/25
...
v = TOR.v;
Cname = TOR.Cname;   % {'tor 1','tor 2','tor 3','tor 4','tor 5','tor 6'};
Sstr  = 'Variable';

disp(['tor_spm_get_ons: Building onset times starting at TOR.c{' num2str(torindex) '}']) 
TOR.vardurations = 0;
TOR.Ptype = 'none';
TOR.Etype = 'linear';
...
if v    
	...
	%	 'time',...
	%	 'other'};
	%TOR.Ptype = spm_input('parametric modulation','+1','b',TOR.Ptype,...
	...
	%	 'exponen',...
	%	 'polynom'};
	TOR.Etype = spm_input('expansion','+1','b',TOR.Etype,'batch',...
	                 {'parametrics',s},'exp_type');
	DSstr = [DSstr  '[ x ' Pstr ' (' TOR.Etype ')] '];
	switch TOR.Etype

		case 'exponen'
		%--------------------------------------------------------
		if strcmp(TOR.Ptype,'time')
			h = round(k*T*dt/4);
