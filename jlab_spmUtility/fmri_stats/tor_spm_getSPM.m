function [SPM,VOL,xX,xCon,xSDM] = spm_getSPM(TOR)
% computs a specified and thresholded following parameter estimation
% FORMAT [SPM,VOL,xX,xCon,xSDM] = spm_getSPM(TOR);
%
...
%-----------------------------------------------------------------------
swd    = TOR.resdir;  % spm_str_manip(spm_get(1,'SPM.mat','Select SPM.mat'),'H');
...
%-----------------------------------------------------------------------
if TOR.maskWOtherContrasts		% spm_input('mask with other contrast(s)','+1','y/n',[1,0],2)
...
titlestr = str;	% spm_input('title for comparison','+1','s',str);
...
if ~isempty(XYZ)
    ...
    d = TOR.multCompCorrect; % spm_input('corrected height threshold','+1','b',str,[],3);
    if strcmp(d,'FWE')
	u  = TOR.u;	% spm_input('FWE-corrected p value','+0','r',0.05,1,[0,1]);
	u  = spm_uc(u,edf,STAT,xSDM.R,n,xSDM.S);
    elseif strcmp(d,'FDR')
	u  = TOR.u;	% spm_input('FDR-corrected p value','+0','r',0.05,1,[0,1]);
	u  = spm_uc_FDR(u,edf,STAT,n,VspmSv,0);
    else
	%-NB: Uncorrected p for conjunctions is p of the conjunction SPM
	u  = TOR.u;	% spm_input(['threshold {',STAT,' or p value}'],'+0','r',0.001,1);
...
if ~isempty(XYZ) & length(Ic) == 1
    ...
    %-------------------------------------------------------------------
    k     = TOR.k;	% spm_input('& extent threshold {voxels}','+1','r',0,1,[0,Inf]);
