...
if isempty(D)
	...
	%---------------------------------------------------------------
	if strcmp(TOR.multivariate,'y') % spm_input('Multivariate analysis?','+1','y/n',[1,0],2)
		nVar = spm_input('number of variates?','+1','n1');
	end

	D    = tor_spm_spm_ui( ...
		char('DesDefs_Stats')); % TOR: load basic stats defaults
    
        %spm_input('Select design class...','+1','m',...
		%{'Basic stats','Standard PET designs','SPM96 PET designs'},...
		%{'DesDefs_Stats','DesDefs_PET','DesDefs_PET96'},2)));
...
% this does not work b/c D is input for type 99, but defined by defaults otherwise.
%if isempty(D), D = D(TOR.destype);, end
    %D(spm_input('Select design type...','+1','m',{D.DesName}'));
...
%-----------------------------------------------------------------------
% put # of pairs in D.n(3) for paired t-test or # imgs in D.n(1) for one-sample
if TOR.destype == 5, 
    D.n(1) = TOR.npairs;
elseif TOR.destype == 2,    % 2-sample t-test
else
    D.n(TOR.destype) = TOR.npairs;
end

[P,I] = tor_spm_spm_ui('Files&Indices',D.sF,D.n,D.b.aTime,nVar,TOR);
% P is empty
P = TOR.P;
...
for i=1:2			% 1:covariates of interest, 2:nuisance variables
	...
	if nC(i)==1, str=dstr{i}; else, str=sprintf('%s %d',dstr{i},nc(i)+1); end
        c = TOR.covariate;  % spm_input(str,GUIpos,'r',[],[nScan,Inf]);
	...
	if nC(i)>1,	tstr = sprintf('%s^{%d}',dcname{i},nc(i));
       	...
       	cname  = TOR.covname;   % spm_input([str,' name?'],'+1','s',tstr);
...
elseif length(D.iGMsca)==1
	...
	iGMsca = TOR.gmscale;
    %spm_input('GMsca: grand mean scaling','+1','m',...
...
%this section is not run if threshold masking is not desired.
% can't figure out how to get it to work if threshold masking is desired (1/23/02;8pm-CY)
if M_T ~= -Inf
...
if ~spm_type(type,'nanrep')
	...
	case Inf,    
        if ~isfield(TOR,'M_I')
            TOR.M_I = 0 %spm_input('Implicit mask (ignore zero''s)?',...
			 %'+1','y/n',[1,0],1);		%-Ask
        end
        M_I = TOR.M_I;
            %= spm_input('Implicit mask (ignore zero''s)?',...
			% '+1','y/n',[1,0],1);		%-Ask
...
if M_X, M_P = TOR.MaskImg;, else, M_P = {};, end
...
spm('Pointer','Arrow')
fprintf('%-40s: %30s\n','Completed',spm('time'))                     %-#
if TOR.estnow   % spm_input('estimate?','_+0','b','now|later',[1,0],1)
	spm('Pointer','Watch')
	spm('FigName','Stats: estimating...',Finter,CmdLine);
	
    % TOR: mod for spm2
    %TSPM.VY = VY; TSPM.xX = xX; TSPM.F_iX0 = F_iX0; TSPM.xC = xC; TSPM.xsDes = xsDes;
    %spm_spm(TSPM);
    spm_spm99(VY,xX,xM,F_iX0,xC,xsDes)
...
if nargin<6, TOR = []; else, TOR = varargin{6}; end
...
for j4  = 1:n4
	...
	for j3 = 1:n3
	    ...
	    for v=1:nV
	    	...
	    	ttP = TOR.P;    % spm_get(n21,'.img',{[str,'select images',vstr]});
	    	n21 = length(ttP);
            %n21 = size(TOR.P,1);
	    	tP  = [tP,ttP];
	    end
	    ti2 = TOR.groupvec';     % spm_input([str,' ',DsF{2},'?'],GUIpos2,'c',ti2',n21,Dn(2));
	    %-Work out i1 & check
	...
	if n2==1 & Dn(1)==1 %-single scan per f3 (subj)
		...
		if Dn(1)==1
            ...
            %end
			%P   = [P;tP];
            P = [];     % put this in later, after running this subfunct!
			i4  = [i4; j4*ones(n2,1)];
			i3  = [i3; j3*ones(n2,1)];
			i2  = [i2; [1:n2]'];
			i1  = [i1; ones(n2,1)];
