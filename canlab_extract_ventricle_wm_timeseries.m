...
% function [nuisance, nuisance_comps] = canlab_extract_ventricle_wm_timeseries(mask_image_dir, imgs, [optional inputs])
%
...
if isa(imgs, 'fmri_data')
    vdat = apply_mask(imgs, vmask);
elseif isa(imgs, 'char')
    vdat = fmri_data(imgs, vmask);
else
    error('The input (imgs) cannot be identified');
end
