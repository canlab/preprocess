...
% model 17: same as model16 (l vs r) but with left only regressor - avoid colinearity.
				% see special code.  no time derivative either.
...
for JJ = 1:length(subs)
   ...
   % special code for model 17
   disp('SPECIAL FOR MODEL 17: REMOVING ROW 2 FROM EVT TIMES MATRIX.')
   for whichR = 1:length(a)
      a{whichR}(2:3,:) = a{whichR}(3:4,:);
      	a{whichR}(4,:) = [];
   end
   disp('new MODEL 17:')
   a{end}(1:3,1:5) 
...
TOR.Etype='linear';   
TOR.Rov='events';  
TOR.Cov=1;		% 1 is hrf only, 2 is hrf with time derivative
TOR.c=c ;
TOR.v = 3;		% number of conditions/trial types in each scan
TOR.Cname = {'tor R','tor Probe1','tor Probe2'};	% names for each trial type
TOR.Volterra = 0;
TOR.userc = 0;
TOR.Global = 'None';
TOR.cLF = 'specify';
