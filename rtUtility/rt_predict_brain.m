function [yhat,yhi,loTime,highTime,x] = rt_predict_brain(y,ons,RTs,p,varargin)
% [yhat,yhi,loTime,highTime,x] = rt_predict_brain(y,ons,RTs,p,varargin)
%
% p = params
...
% RTs, modulator values, preferably centered
...
%                       RT (with centered RTs)
%
% p = [1 0 0 1 0];  % "Variable epoch": convolve RT with duration 
%                       (with non-centered, raw RTs).  Parametric
...
% event signal  duration = p3 + RTs * p4
%
% OPTIONAL INPUTS
% -------------------------------------------------------------------------
...
% ------------------------------------------------------------------------
% y = rand(330,1);
% ons = [1:20:320]'; RTs = rand(size(ons));
% [yhat,yhi] = rt_predict_brain(y,ons,RTs,[1 1 1 0 0],'plot');  % linear RT modulation of height
% [yhat,yhi] = rt_predict_brain(y,ons,RTs,[1 0 1 10 0],'plot');  % linear RT modulation of duration
% 
% [yhat,yhi] = rt_predict_brain(y,ons,RTs,[.5 1 1 0 0],'plot');  % other linear RT modulation of height
% [yhat,yhi] = rt_predict_brain(y,ons,RTs,[.5 .7 1 5 0],'plot');  % saturated RT modulation of height; linear width
%
...
% fhan = @(p, times) rt_predict_brain(y,ons,RTcenter,p);
%
...
% magnitude = p(1) .* RTs .^ p(2); % p(2) .^ RTs; %RTs .^ p(2);
% duration = relSampRate .* (p(3) + p(4) .* RTs); % in hi-res samples ; could be .* RTs, but then neg. durations are possible?

% want RT effects to be independent from overall trial effects
% with centered RTs, this should be accomplished by:
magnitude = p(1) + p(2) .* RTs;  % .^ p(2); 
duration = relSampRate .* (p(3) + p(4) .* RTs);
% but RTs must be positive to use exponents, e.g., RTs .^ p(2), otherwise fractional p's will
% give imaginary numbers.

% specify 'neural' boxcars based on mag and dur
x = get_boxcars(ons,x,magnitude,duration);

% convolve
yhi = p(5) + fast_conv_fft(hrf,x);
...
% END MAIN FUNCTION
    ...
    function setup_random
        ...
        b=.13;
        minRT=.5;
        RTs=gamrnd(a,b,numTrials,1)+minRT;
    ...
    function setup_vars
        ...
        % Check
        if length(RTs) ~= length(ons)
            error('Length of RTs and length of ons must match.');
        end
...
for i = 1:length(ons)
    % for each onset
    x(ons(i) : ons(i) + duration(i) - 1) = magnitude(i);
end
