...
%numc = 148;
[d] = textread(fname,'%s','headerlines',hdrlines,'delimiter','\t');

if strcmp(d{1},'This file contained edited data.'),
    % add an extra input row!
    disp('edited data: extra row.')
    addone = 1;
    [d] = textread(fname,'%s','headerlines',hdrlines+1,'delimiter','\t');
end
...
if addone
    str = [outs ' = textread(''' fname ''',fmt,''headerlines'',' num2str(hdrlines+1) ',''delimiter'',''\t'');'];
else
    str = [outs ' = textread(''' fname ''',fmt,''headerlines'',' num2str(hdrlines) ',''delimiter'',''\t'');'];
end
