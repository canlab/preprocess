...
%
% [y,y_mean] = voistat('adjusty',y,[high pass freq. cutoff],[sampling rate in s])
...
zeromagnitude = [];
	...
	for j = 1:stop-avgover
		...
		if ~isempty(mybase) & ~isempty(trialy) & ~(sum(includey == length(trialy))),
					...
					%mybase = y(j);, % warning(['Group ' num2str(g) ' image ' num2str(j) ':mybase empty: using first timepoint in trial.'])
			...
			% debugging - or cool for showing individual trials.  build in 'show' option in the future.
			%figure;
			%plot(trialy); hold on; plot(avgd(index,:),'r'); plot([1 60],[zeromagnitude(index) zeromagnitude(index)],'k')
			%pause
			%close
...
% debugging
% 06/05/01 analysis uses all trials.  IS converting to change from baseline.
% also, trimming is not occurring in the avgd - only in the overall averages.  must nan out outliers in above section.
% ...but this may not be bad, as results look good and easier to explain.  so maybe i'll NOT do trimming.
% use:  [c,b,a] = voistat('trialavg',ROI.adjustedy,avgover,scolors,[],events,groups,basepoints,dosteplot,outthresh);
...
filtmethod = 'spm';
	% options are: spm, cheby, fourier, Luis
switch filtmethod
...
[y,ntrimmed] = trimts(y,3,[]);
fprintf(1,'		...Windsorized %3.0f outliers of %3.0f points (%3.0f%%) at 3 standard deviations.\n',ntrimmed,length(y),100*ntrimmed./length(y));


varargout{1} = y;
varargout{2} = wholeyavg;               % overall mean before filtering
