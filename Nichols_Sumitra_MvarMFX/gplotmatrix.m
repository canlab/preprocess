...
if (~isempty(xnam))
   if (size(xnam,1) ~= cols)
      error('XNAM must have one row for each column of X.');
   end
end
if (~isempty(ynam))
   if (size(ynam,1) ~= rows)
      error('YNAM must have one row for each column of Y.');
   end
elseif (XvsX)
   ynam = xnam;
end
...
if ndims(x)>2 | ndims(y)>2, error('X and Y must be 2-D.'); end
if size(x,1)~=size(y,1)
  error('X and Y must have the same length.');
end
if (length(g)>0) & (length(g) ~= size(x,1)),
  error('There must be one value of G for each row of X.');
end
