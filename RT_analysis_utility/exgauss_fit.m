function [parmhat, parmci] = exgauss_fit(x, alpha, censoring, freq, options)
%GAMFIT Parameter estimates for gamma distributed data.
%   PARMHAT = exgauss_fit(X) returns maximum likelihood estimates of the
%   parameters of an ex-gaussian distribution fit to the data in X.  PARMHAT(1)
...
if ncen == 0
    ...
    % Otherwise, find MLEs.
        ...
        options = statset(statset('exgaussfit'), options);
        ...
        if lkeqn(ahat, const) > 0
            ...
            while lkeqn(lower, const) > 0
                ...
                if lower < realmin(class(x)) % underflow, no positive root
                    error('stats:exgaussfit:NoSolution',...
...
% strictly positive, with a full-length censoring vector.
    ...
    funfcn = {'fungrad' 'exgaussfit' @negloglike [] []};
