...
betaflag = 1;       % betaflag: if 1, no Fishers r to z; if 0, assumes correlations

transs = {'On' 'Off'};
fprintf(1,'\nxc_stats: Statistics on correlations/betas.  Fisher''s r to Z is: %s\n)',transs{betaflag});
...
for j = 1:size(xc,2)-1
    for k = j + 1 : size(xc,2)
        ...
        tmp = squeeze(xc(j,k,:));
        if betaflag,
            % do nothing; zs are beta weights
            z = tmp;
        else
            % assume correlation
...
fprintf(1,'Corr T-values, * = p < .05 uncorrected, 2-tailed\n');
[str, sigu] = correlation_to_text(tcorr,crit_tu,names);
OUT.sigmat_uncorrected = sigu;

fprintf(1,'Corr T-values, * = p < .05 corrected, 2-tailed\n');
