...
OLDF=f;
FIRSTF=f;
[OLDX,OLDF,HESS,OPTIONS]=optint1(XOUT,f,OPTIONS);
...
%   ...interpolate.
        how='inter';
        [stepsize]=cubici11(f,FIRSTF,gdnew,GDOLD,OPTIONS(18));
        ...
        XOUT=OLDX;
    elseif f<FIRSTF
        [newstep,fbest] =cubici31(f,FIRSTF,gdnew,GDOLD,OPTIONS(18));
...
%         .... update and calculate new direction.
        how='';   
            ...
            [HESS,SD]=updhess1(XOUT,OLDX,GRAD,OLDG,HESS,OPTIONS);
            gdnew=GRAD'*SD;
            OLDX=XOUT;
            status=4;
...
% Line search using mixed polynomial interpolation and extrapolation.
        if PCNT~=0 
            while PCNT > 0 & OPTIONS(10) <= OPTIONS(14)
                ...
                [PCNT,MATL,MATX,steplen,f, how]=searchq1(PCNT,f,OLDX,MATL,MATX,SD,GDOLD,OPTIONS(18), how);
                OPTIONS(18)=steplen;
