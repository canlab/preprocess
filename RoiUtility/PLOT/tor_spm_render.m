function RGB = tor_spm_render(dat,brt,rendfile,colors)
% Render blobs on surface of a 'standard' brain
% FORMAT tor_spm_render(dat,brt,rendfile,colors)
%
...
% surface.
%_______________________________________________________________________
% @(#)spm_render.m	2.16 John Ashburner FIL 99/05/18
...
%=======================================================================
if nargin<1
	SPMid = spm('FnBanner',mfilename,'2.16');
...
if ~finite(brt),
	...
	disp('0 = r g b, 1 = b g r, 2 = r b g')
	tor_swap = input('Swap colors?');
	...
	for i=1:length(rend),
		...
		RGB{i} = rgb;
		% tor custom color assignment
		% swapping :,:,1 with another layer changes the color
		% if strcmp(colors{i},'r')
