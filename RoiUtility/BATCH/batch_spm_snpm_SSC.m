...
% FORMAT spm_snpm_SSC 
%
% See body of spm_snpm_ui for definition of PlugIU interface.
...
%_______________________________________________________________________
% @(#)spm_snpm_SSC.m	2.2 Andrew Holmes 01/06/23
...
%-----------------------------------------------------------------------
P = BAT.P;  %P     = spm_get(+Inf,'.img','Select scans in time order');
nScan = size(P,1);
...
while ~all(size(CovInt)==[nScan,1])
	CovInt = BAT.behavior; %CovInt = spm_input(sprintf('Enter covariate values [%d]',nScan),'+1');
	%CovInt = CovInt(:);
end
...
if length(Xblk)>1
  Xblk     = BAT.Xblk; %spm_input('Size of exchangability block','+0','b',tmp,Xblk);
end
nXblk    = (nScan/Xblk);
...
bAproxTst = BAT.bAproxTst;  %bAproxTst = spm_input(sprintf('%d Perms. Use approx. test?',nPiCond),...
							%'+1','y/n')=='y';
if bAproxTst
    tmp = 0;
...
%-Design description
%-----------------------------------------------------------------------
sDesign = sprintf('Single subject correlation: %d scans',nScan);
