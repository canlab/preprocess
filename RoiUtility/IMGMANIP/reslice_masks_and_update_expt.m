...
if reslice1sts, applynow = input('Masks already applied (0) or need to be applied to 1st subject now (1)? ');, end
...
if doreset
    disp(['Changing 1st image name in EXPT.SNPM.P{:}!'])
    for conindex = 1:length(EXPT.SNPM.connums)
        myfile = deblank(EXPT.SNPM.P{conindex}(2,:)); 
        [myd] = fileparts(deblank(EXPT.SNPM.P{conindex}(1,:))); 
        %[myd,f,e] = fileparts(fileparts(deblank(EXPT.SNPM.P{conindex}(1,:))));
        EXPT.SNPM.P{conindex} = str2mat([myd filesep myfile(end-11:end)],EXPT.SNPM.P{conindex}(2:end,:));
    end
    
    if ~(exist(EXPT.SNPM.P{1}(1,:)))
        myd = spm_get(-1,'con*img','Select dir containing 1st subject con*img');
        for conindex = 1:length(EXPT.SNPM.connums)
            myfile = deblank(EXPT.SNPM.P{conindex}(2,:)); 
            EXPT.SNPM.P{conindex} = str2mat([myd filesep myfile(end-11:end)],EXPT.SNPM.P{conindex}(2:end,:));
        end
