...
% Subject is fixed effect, unless you use tor's modified function
%
...
EXPT.ANOVA.numfactors = factor_index;
EXPT.ANOVA.levels = max(cat(2,EXPT.ANOVA.factorcodes{:}));

a = repmat(EXPT.DX.dxtrialonsets,EXPT.DX.numframes,1);
b = repmat((1:EXPT.DX.numframes)'-1,1,size(EXPT.DX.dxtrialonsets,2));
c = a + b; 
EXPT.DX.paramsofinterest = c(:);
