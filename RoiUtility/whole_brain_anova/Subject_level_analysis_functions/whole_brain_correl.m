% function Pw = whole_brain_correl(P,seeds,DX,TR,HP,nruns,numframes,[doplot],[mask],[smoothlen],[startslice])
%
...
%       below)
% DX = model matrix of effects to REMOVE before correl, see, e.g., for FIR tor_make_deconv_mtx3.m
% TR = repetition time of your study (sampling rate)
% HP = high-pass filter cutoff in seconds to apply (SPM style)
% nruns = num. of images in each sess,
%           e.g., [169 169 172], same as numframes
...
function Pw = whole_brain_correl(P,seeds,DX,TR,HP,nruns,numframes,varargin)
    % -------------------------------------------------------------------
    ...
    dolindetrend = 1; % linear detrending on whole timeseries

    % model fitting
    % ---------------
    docorrel = 1;     % correl analysis and image writing
    ...
    switch type
        case 'full', % do nothing
            ...
            fprintf(1,'\nCORREL SEED: no smoothing, no writing filtered imgs\n')
    ...
    if dofilt
        ...
        clear dummy KL
        X = intercept_model(numframes); % nruns has number of images in each run
    ...
    [dummy,f,e]=fileparts(deblank(P(1,:)));
    basedir = pwd;
    V(1).fname = fullfile(basedir,[f e]);
    V(1).private.dat.fname = V(1).fname;
    ...
    for slicei = firstslice:dims(3)
        ...
        [a,b] = process_slice(slicei,seeds,P,S,PSDXS,TR,V,Vall,F,numframes,mask(:,:,slicei),smoothlen);
...
function [fbetas,ntrimmed] = process_slice(slicei,seeds,P,S,PSDXS,TR,V,Vall,F,numframes,varargin)
    ...
    if dolindetrend
        fprintf(1,'\n\tLinear detrending data for %3.0f voxels > ',length(i))
...
%         clear sl  Schafer: sl is used below under dofilt, dographics
    ...
    if dofilt
        ...
        for k = 1:length(i)
            ...
            % first image usually off -- replace w/mean of first 10 images
            y = squeeze(fsl(i(k),j(k),:)); y(1) = mean(y(min(length(y),10)));
    ...
    if writefilt
        ...
        for img = 1:size(P,1)
            ...
            basedir = pwd;
            V(img).fname = fullfile(basedir,['f' f e]);
            V(img).private.dat.fname = V(img).fname;
    ...
    if ~isempty(PSDXS),
        ...
        fprintf(1,'\n\tFitting model matrix (to get residuals, nOT IMPLEMENTED YET) > ')
    ...
    % -------------------------------------------------------------------
    % * correl analysis on residuals (or data if no model is entered)
    % -------------------------------------------------------------------
    if docorrel
        ...
        shiftval = NaN * zeros([dims(1:2) nseeds]);
        rval = shiftval;
        zval = shiftval;

        fprintf(1,'\n\tFitting correl model, Max shift = %3.0f, Shift by = %.03f TRs, Robust = %3.0f, Betaflag = %3.0f, %3.0f separate seeds > ', maxshift, shift_step, robustflag, betaflag, size(seeds,2))
        ...
        for k = 1:length(i)
            ...
            for R = 1:nseeds
                %[sval,myc,bestval,bestcor,xout,yout] = shift_correl(x,squeeze(fsl(i(k),j(k),:)),[shiftbymax],[robustflag],[betaflag],[doplot])

                [sval,myc,bestval,bestcor,xx,yy] = shift_correl(seeds(:,R), squeeze(fsl(i(k),j(k),:)), 'max_shift', maxshift, 'shift_step', shift_step, 'use_rob_stats', robustflag, 'return_betas', betaflag, 'display_plots', 0);

                % shouldn't happen unless there's a perfect correlation, but
                % just in case.  perfect correl should only happen if we cut
                % off almost all the variance in one of the timeseries, and there are too few df.
                % this could happen for voxels that are only in-brain for a few
                % time points ... deserves a bit more checking into why these
                % exist.
                if length(bestval) > 1, bestval = bestval(abs(bestval) == min(abs(bestval)));, end
                if length(bestval) > 1, bestval = bestval(1);,end
                if isempty(bestval), bestval = NaN;,end

                shiftval(i(k),j(k),R) = bestval;
                rval(i(k),j(k),R) = bestcor;
            end

            zval(i(k),j(k),:) = fisherz(rval(i(k),j(k),:));

            %if any(isnan(shiftval(i(k),j(k),:))), disp('Some betas are NaN for non-empty voxels.  This should not happen.'); keyboard, end
        ...
        warning on
        if dographics,
            figure(f1); subplot 223; imagesc(rval); title('Correl values'),colorbar('horiz'),drawnow;
            figure(f1); subplot 224; imagesc(shiftval); title('Correl shift'),colorbar('horiz'),drawnow;

            saveas(gcf,['correl_image_' num2str(slicei)],'tif')
        end
        ...
        % * write Correl output
        % -------------------------------------------------------------------
        fprintf(1,'\n\tWriting Correl output > ')
        et = clock;

        emptyimg = zeros(dims);     % in case we need to create a new volume

        % write images for all beta series
        write_beta_slice(slicei,V(1),shiftval,emptyimg,['correl_shift_']);
        write_beta_slice(slicei,V(1),rval,emptyimg,['correl_r_']);
        write_beta_slice(slicei,V(1),zval,emptyimg,['correl_z_']);
...
function Pw = write_beta_slice(slicei,V,betas,emptyimg,varargin)
    ...
    %V.dim(4) = 16; % set to float to preserve decimals  %breaks spm_write_vol
    ...
    for voli = 1:size(betas,3) % for each image/beta series point
        ...
        V.private.dat.fname = [prefix myz num2str(voli) '.img'];
        V.dt = [16 0];
        V.private.dat.dtype = 'FLOAT32-LE';
        V.descrip = ['Beta image for column ' num2str(voli)];

        % fix in case we're basing off of 4-D images
        V.dim = V.dim(1:3);
        V.dt(1) = 16; % 'float32' - fix in case using inferior data type
        
        % create volume, if necessary
        if ~(exist(V.fname) == 2), spm_write_vol(V,emptyimg);
        end
