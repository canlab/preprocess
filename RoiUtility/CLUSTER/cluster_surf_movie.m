...
set(gcf,'Color','w');axis off; %view(230,10)
lightfollowview, material dull

a = actcolors{1}; actcolors{1} = a .* .5;
O = struct('add2movie',[],'zoom',1.3,'azOffset',[],'elOffset',[],'timecourse',[],'fps',5,'length',6);
O.name = 'cluster_surf.avi';
...
if isempty(O.add2movie)
    try
        mov = avifile(O.name,'Quality',75,'Compression','Indeo5','Fps',O.fps);
    catch
...
colmult = [.5:1./(numFrames-1):1];
colmult = [colmult 1:-1./(numFrames-1):.5];
[az,el] = view;
...
catch, lasterr, warning('There was an error making this movie file.  No frames written.')
    mov = close(mov);
end
...
% -------------------------------------------
view(0,90)
O = struct('add2movie',[],'zoom',1,'azOffset',180,'elOffset',90,'timecourse',[],'fps',5,'length',6);
colmult = [.5:1./(numFrames-1):1];
colmult = [colmult 1:-1./(numFrames-1):.5];
[az,el] = view;
if ~isempty(O.azOffset),az = az - O.azOffset;,az2add = O.azOffset ./ numFrames;,end
