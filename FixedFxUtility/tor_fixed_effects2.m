function [clusters,vol,maskingImage,V] = tor_fixed_effects2(connum,crit_p,cl_size)
% [clusters,vol,maskingImage,V] = tor_fixed_effects2(connum,crit_p,cl_size)
% shell for running fixedfx2, fixed effects analysis
%
% start in directory above individual results directories containing spmT*imgs
...
if connum < 10, myz = '000';, else, myz = '00';,end
conname = ['spmT_' myz num2str(connum) '.img'];
D = dir;
fnames = [];
for i = 1:length(D)
    if (exist([D(i).name filesep conname]) == 2)
        fnames = str2mat(fnames,[D(i).name filesep conname]);
    end
end
fnames = fnames(2:end,:);
nimgs = size(fnames,1);
...
if isstruct(V)
	...
	V2 = V;
	V2.fname = ['fixed_results_' myz num2str(connum)];
