function params = spm_normalise_mod(VG,VF,matname,VWG,VWF,flags, T)
...
elseif finite(flags.cutoff) & flags.nits & ~isinf(flags.reg),
	...
	Tr = snbasis(VG1,VF1,VWG,VWF,Affine,...
		max(flags.smoref,flags.smosrc),flags.cutoff,flags.nits,flags.reg,T);
else
	Tr = [];
end;
clear VF1 VG1
...
if ~isempty(matname),
	...
	save(matname,'Affine','Tr','VF','VG','flags','T');
end;
return;
%_______________________________________________________________________

%_______________________________________________________________________
function Tr = snbasis(VG,VF,VWG,VWF,Affine,fwhm,cutoff,nits,reg,T)
...
if nargin < 7 || isempty(T)
    ...
    disp('Using starting estimates for T in spm_normalise_mod');
    % we have input T
    % NEED TO FIX
    % reshape Tr input to T
%     T = [Tr(:) ./ stabilise.^3; zeros(prod(size(VG))*4,1)] ;
end
